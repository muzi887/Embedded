...
const u32 memblksize = MEM1_BLOCK_SIZE;		  // 内存块大小
...
void mem_init(void)
{
	...
	mymemset(mallco_dev.membase, 0, memsize);		// 内存池数据清零
...
u32 mem_malloc(u32 size)
{
	...
	if (size % memblksize)
		nmemb++;
	for (offset = memtblsize - 1; offset >= 0; offset--) // 从最高地址开始查找
	{
...
u8 mem_free(u32 offset)
{
	...
	if (offset < memsize) // 偏移在内存范围内.
	{
		index = offset / memblksize;	  // 偏移所在内存块号
...
void *myrealloc(void *ptr, u32 size)
{
	...
	else
	{
		...
		myfree(ptr);													 // 释放旧内存
		return (void *)((u32)mallco_dev.membase + offset);				 // 返回新内存起始地址
	}
}
