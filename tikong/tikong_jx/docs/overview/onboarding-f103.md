...
4. **迁移笔记**（建议自写半页）：
  - `board_init()` 里 `uart1_init` / `RS485_Init` / `uart3_init` / `uart4_init` / `uart5_init` 与 F103 单路 `Serial_Init` 的对应关系
    - **每调一次 ≈ 再做一遍** `Serial_Init`
  - `delay_init(168)` 与蓝 pill `72` 的差别
    - **蓝 pill（Blue Pill）** 是市面上很常见的一块 **STM32F103C8T6** 入门开发板：小板、便宜、板子常为蓝色
    - `delay_init(SYSCLK)` 里的参数是 **系统主频（MHz）**，用来算 SysTick 延时倍乘（`fac_us` / `fac_ms`），让 `delay_ms` / `delay_us` 走时准确。
    - `168`/`72` 只服务 **软件延时**（以及依赖该 delay 的初始化间隔）。
...
1. 精读入口链：
  ...
  - `User/app_run.c`：`app_init`（EEPROM/RTC/楼层限高）与 `app_poll`
...
另：`main` 在 `g_uart3_at_sequence_request` 置位时走 **4G AT 配置序列**（配网/写密钥），完成后复位；正常业务在 `else` 分支的 `app_poll()`。流程见 [at-config-flow.md](../cloud/4g/at-config-flow.md)。
...
4. 流程图：[main-flowchart.mmd](./main-flowchart.mmd)；日常轮询见 [app-poll-flow.md](./flows/app-poll-flow.md)。
5. 抽查一路：DMA 空闲中断如何置 `RX_Complete`，再进 `*Process()`。完整路径见 [uart4-dma-rx.md](../serial/uart4-dma-rx.md)（入口为 `QRProcessUart4`）。
...
资料与阅读节奏以专文为准（**不必通读 PDF**）：

→ **[pdf-reading-order.md](../cloud/4g/pdf-reading-order.md)**（阶段 D 导读）  
→ **[source-map.md](../cloud/4g/source-map.md)**（main 分叉：配网路 / JSON 路）

建议步骤：

1. 源码入口：`App/Cloud/g4g_comm.c` → `App/Cloud/Live_data_r.c` 的 `parseSerialData`。
2. AT 配网：`App/Cloud/uart3_at_sequence.*`；模组驱动：`Hardware/Modem/4G.*`；整流程见 [at-config-flow.md](../cloud/4g/at-config-flow.md)。
3. 先锁定平台 **物模型（TSL）** key，再追固件：
   - 表格摘要：[thing-model-v2.md](../cloud/thing-model-v2.md)
   - 完整 JSON：[thing-model-v2.json](../cloud/thing-model-v2.json)
4. 任选一条服务追到底（推荐）：
   - 黑名单：`blacklist_add` 等（见物模型服务表）
   - 或 RS485 透传样例：[parseSerialData-case101.md](../cloud/parseSerialData-case101.md)
5. 联调需 4G 模组或串口模拟 `数字,JSON`；注意主循环若在跑 AT 序列，则不会进入 `app_poll`。

**复用你的知识**：USART3 仍是串口；MQTT Topic 在模组/协议层，固件侧主要是 **收 JSON 字符串 → 解析 → 执行 → 回 JSON**。物模型 TSL 定义「有哪些事件/服务、字段叫什么」；`parseSerialData` 的 case 数字是 MCU 侧入口编号，二者对照着看。
...
地址表与驱动 API 见 [eeprom.md](../store/eeprom.md)。
...
8. cloud/4g/pdf-reading-order.md → source-map.md → g4g_comm / Live_data_r（云端）
9. cloud/thing-model-v2.md（+ .json TSL）锁定 key 后再追 parseSerialData

【联调细节】
10. docs/store/storage-logic.md（阶段 E）；uart4-dma-rx / eeprom / parseSerialData-case101 等按需
```
...
4. 云端路径固件侧入口函数是谁？物模型文件是什么？
  → `G4GProcess` → `parseSerialData`；平台功能定义是 TSL：`docs/cloud/thing-model-v2.json`。
