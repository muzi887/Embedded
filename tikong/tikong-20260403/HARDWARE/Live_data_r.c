...
int parse_upgongyao_json(cJSON *root)
{
    ...
    if (publicKey_6[0] != '\0')
    {
        strncpy(g_device_publicKey, publicKey_6, sizeof(g_device_publicKey) - 1);
        g_device_publicKey[sizeof(g_device_publicKey) - 1] = '\0'; // 确保字符串以 '\0' 结尾
    }
